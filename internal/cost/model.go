...
// multipliers maps runner labels to GitHub's published billing multipliers.
// GitHub bills per-minute with different rates per runner OS/size.
// Rates last verified 2026-07-17.
...
// Checks each label against the known multiplier table, then larger-runner
// pattern matching, then an OS-prefix fallback. Returns 1.0 (Linux default)
// if nothing matches.
func LookupMultiplier(labels []string) float64 {
	...
	for _, label := range labels {
		...
		if mult, ok := multipliers[label]; ok {
			return mult
		}
	...
	// GitHub images newer than the pinned table (the next "macos-NN") must
	// not silently bill at the 1x Linux default — a 10x underestimate for
	// macOS. Vendor labels without a GitHub OS prefix still fall through.
	for _, label := range labels {
		if mult, ok := osPrefixMultiplier(strings.ToLower(label)); ok {
			return mult
		}
	}
	return 1
...
// osPrefixMultiplier prices a label by its GitHub OS prefix alone.
func osPrefixMultiplier(label string) (float64, bool) {
	switch {
